...
class TrainFittedPreprocessor(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, feature_order, categorical_specs=None):
        # scikit-learn's clone() rebuilds this object from exactly these two
        # arguments, so they are stored verbatim and nothing is derived here.
        # Storing derived state in __init__ would silently break cloning, and
        # cloning is what gives every cross-validation and calibration fold its
        # own independently fitted preprocessor.
        self.feature_order = feature_order
        self.categorical_specs = categorical_specs
        self.medians_ = {}
        self.zero_filled_cols_ = []
        self.missing_in_training_cols_ = []
        self.categories_ = {}
    ...
    def numeric_cols(self):
        """Design-matrix columns that are not categorical expansion points."""
        sources = {spec["source"] for spec in (self.categorical_specs or [])}
        return [c for c in self.feature_order if c not in sources]
    ...
    def fit(self, frame, y=None):
        ...
        When this object is used inside a scikit-learn pipeline, ``frame`` is
        the fold's TRAINING slice, so the medians and categorical schema below
        are learned from that fold alone and never from its validation rows.
        ``y`` is accepted and ignored so the signature matches the scikit-learn
        transformer API.
        """
        sources = {spec["source"] for spec in (self.categorical_specs or [])}
        ...
        # A median is learned for EVERY numeric column, not only for those that
        # happen to contain a missing value in this particular training slice.
        #
        # Recording it only on demand was safe while one preprocessor was fitted
        # on the whole outer-training set, because any column with missingness
        # anywhere in training got a median. Once each fold fits its own
        # preprocessor that no longer holds: a column can be complete in a
        # fold's training rows and missing in its validation rows, leaving
        # nothing to impute with and failing the transform. Learning the median
        # unconditionally fixes that using only this slice's own values, so the
        # fold-isolation guarantee is untouched.
        #
        # A column with no observed value at all in the training slice carries
        # no information; it is filled with 0.0 and recorded in
        # ``zero_filled_cols_`` so the substitution is visible rather than
        # silent.
        self.medians_ = {}
        self.zero_filled_cols_ = []
        self.missing_in_training_cols_ = []
        for col in self.numeric_cols:
            ...
            if series.isnull().any():
                self.missing_in_training_cols_.append(col)
            median = series.median()
            if np.isfinite(median):
                self.medians_[col] = float(median)
            else:
                self.medians_[col] = 0.0
                self.zero_filled_cols_.append(col)
    ...
    # Application
    # ------------------------------------------------------------------
    def transform(self, frame, partition="unnamed"):  # noqa: D401
    ...
    def fit_transform(self, frame, y=None, partition="train", **kwargs):
    ...
    def schema(self):
        ...
        return {
            ...
            # Columns that actually carried a missing value in this training
            # partition. A median is learned for every numeric column so that a
            # value missing only in a validation or held-out partition can still
            # be imputed, but only these columns required imputation here.
            "imputed_cols": sorted(self.missing_in_training_cols_),
            "columns_with_learned_median": sorted(self.medians_),
            "training_medians": {k: float(v) for k, v in sorted(self.medians_.items())},
            "zero_filled_cols": sorted(self.zero_filled_cols_),
            "categorical_sources": [
                {
                    "source": spec["source"],
