...
def run_fairness_analysis(
    ...
    X_train = preprocessor.transform(work.loc[train_index], partition="train")
    X_test = preprocessor.transform(work.loc[test_index], partition="test")
    ...
    for name, model in models.items():
        try:
            scores = cross_val_score(
                model, X_train, y_train, cv=selection_cv,
    ...
    if clusters is None:
        ...
        calibration_cv = list(
            StratifiedGroupKFold(
                n_splits=5, shuffle=True, random_state=random_state
            ).split(X_train, y_train, groups=clusters_train.to_numpy())
        )
    ...
    prob_uncalibrated = best_estimator.predict_proba(X_test)[:, 1]

    # FINAL, one-time test-set evaluation of the selected model (reporting
    # only; this number was NOT used to choose the model). AUROC is invariant
    # to the monotone isotonic calibration.
    ...
    if verbose:
        print("=" * 70)
        print(f"STEP 5: Bootstrap {int((1 - 0.05) * 100)}% CIs (n_boot={n_boot})")
        print("=" * 70)

    # When the test partition contains repeated encounters per patient, the
    # resampling unit is the patient rather than the encounter, so the
    # interval reflects within-patient correlation. Datasets without a
    # clustering variable keep the existing row-level bootstrap unchanged.
    if clusters is None:
        def _boot(key, **kwargs):
            return adj.bootstrap_gap_ci(
                y_test_arr, prob, pred, mask_a, mask_b, key,
                n_boot=n_boot, seed=random_state, **kwargs
            )
        bootstrap_protocol = BOOTSTRAP_PROTOCOL_ROW
    else:
        cluster_test_arr = clusters_test.to_numpy()

        def _boot(key, **kwargs):
            return adj.bootstrap_gap_ci_clustered(
                y_test_arr, prob, pred, mask_a, mask_b, key,
                cluster_ids=cluster_test_arr,
                n_boot=n_boot, seed=random_state, **kwargs
            )
        bootstrap_protocol = BOOTSTRAP_PROTOCOL_CLUSTER

    boot_results = {}
    raw_metrics_to_boot = ["sensitivity", "specificity", "fpr", "fnr",
                            "ppv", "npv", "predicted_positive_rate", "auroc"]
    for key in raw_metrics_to_boot:
        if key == "auroc":
            continue  # AUROC bootstrap needs probability resampling too; skip for now
        res = _boot(key)
        boot_results[key] = res
        if verbose:
            sig = "*" if res["ci_low"] > 0 or res["ci_high"] < 0 else " "
            print(f"  raw {key:24s} gap = {res['point']:+.4f}  "
                  f"95% CI [{res['ci_low']:+.4f}, {res['ci_high']:+.4f}] {sig}")

    boot_results_adj = {}
    target_prev = prev_adj["target_prevalence"]
    for key in ("ppv", "npv", "predicted_positive_rate"):
        res = _boot(key, prevalence_adjust=True, target_prevalence=target_prev)
        boot_results_adj[key] = res
        if verbose:
            sig = "*" if res["ci_low"] > 0 or res["ci_high"] < 0 else " "
            print(f"  prevalence-adjusted {key:12s} gap = {res['point']:+.4f}  "
                  f"95% CI [{res['ci_low']:+.4f}, {res['ci_high']:+.4f}] {sig}")
    if verbose:
        print("  (* = 95% CI excludes 0, i.e. gap is statistically significant)\n")
