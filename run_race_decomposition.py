...
# One row per bootstrapped raw gap plus one per prevalence-standardized gap.
BOOTSTRAP_ROWS_PER_COMPARISON = (
    len(unc.THRESHOLD_METRICS) + len(unc.PREVALENCE_ADJUSTED_METRICS)
)
...
def stability_check_10seed(df, fc, tc, gc, ga, gb, preprocess_spec=None,
    ...
    """
    from src.pipeline import _make_calibrated_classifier, _model_pipeline
    ...
    for seed in range(10):
        ...
        # Preprocessing is bound to the estimator so each calibration fold
        # refits it on that fold's training rows only, matching the primary
        # pipeline. Fitting one preprocessor on the whole stability-training
        # set and reusing it inside the folds would reproduce, in this
        # secondary check, the leakage corrected in the primary analysis.
        raw_tr = work.loc[tr_idx]
        raw_te = work.loc[te_idx]
        y_tr, y_te = y.loc[tr_idx], y.loc[te_idx]
        g_te = g.loc[te_idx]

        thr = float(y_tr.mean())
        model = _model_pipeline(
            TrainFittedPreprocessor(fc, spec),
        ...
        if clusters is None:
            ...
            calibration_cv = list(
                ...
                .split(raw_tr, y_tr, groups=clusters.loc[tr_idx].to_numpy())
            )
        cal = _make_calibrated_classifier(model, cv=calibration_cv)
        cal.fit(raw_tr, y_tr)
        prob = cal.predict_proba(raw_te)[:, 1]
...
for path, loader_fn in LOADERS:
    ...
    # ── frozen held-out outputs (primary comparison) ───────────────────
    ...
    # respect to the analysis: every value comes from the pipeline result.
    try:
        frozen_manifests.append(
            fo.write_comparison_artifacts(
                art["result"],
                slug=_frozen_slug("race", name),
                dataset=name,
    ...
    try:
        ...
        # training-fitted preprocessing, so it is given RAW rows here.
    ...
    for (sa, sb) in secondary_comparisons:
        ...
        try:
            ...
            # Secondary comparisons are reported in the Supplement, so they
            # are real reported analyses and get the same persisted evidence
            # as every primary comparison.
            frozen_manifests.append(
                fo.write_comparison_artifacts(
                    art_s["result"],
                    slug=_frozen_slug("race", f"{name}_{sa}_vs_{sb}"),
                    dataset=name,
                    comparison_type="race_secondary",
...
expected_counts = {
    ...
    # Derived from the metric list rather than hardcoded, so adding or removing
    # a bootstrapped gap cannot silently break the completeness guard.
    "bootstrap rows": expected_total * BOOTSTRAP_ROWS_PER_COMPARISON,
